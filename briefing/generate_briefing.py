...
3. NEWS: Start from newsletter content, fill gaps with search results, preferring newsletter content. Priority order: world headlines (major items + maybe 1 niche cool one) -> 3 WSJ links (one macro, one company/industry, one op-ed; FT/Economist/NYT acceptable fallback) -> markets & macro -> M&A/PE/dealflow -> tech/AI -> athletics: a CONCISE scoreboard — one tight line per item as "matchup/event — result" (e.g. "Princeton MLAX vs Yale — W 12-10", "Celtics @ Knicks — L 104-110"), NOT prose; cover lacrosse NCAA D1 + pro, football, golf, snow sports, hockey, basketball, baseball; bias Princeton, Bay Area, Boston; only include games that actually have a result -> 3 unique niche cool things -> 3 learning points (>=1 on actionable tech/AI skill or workflow). Skip empty categories rather than padding. Tight, mostly bullets. Only include links you are confident are live.
...
TYPE SCALE (inline font-size in px, explicit line-height in px, margin:0 on headings; Arial/Helvetica for ALL headings and body; Courier New ONLY for the tiny eyebrow label):
  ...
  Subsection (H3):      17px / weight 700 / line-height 23px / color {p['text']}  (must be clearly readable and stand out — NOT muted, NOT Courier New, NOT tiny. Make the TEXT specific and substantive, not a bland one-word category tag: prefer a short descriptive phrase that reflects the actual content/angle over generic labels like "MARKETS" or "TECH".)
