...
6. LENGTH: Target 1000-2500 words of visible text. Do NOT undershoot — this is a full, substantive daily briefing, not a summary. If in doubt, write more, with specifics.
7. VOICE — write like a sharp, well-read human editor, NOT like an AI assistant:
   - NEVER use em dashes (—) or en dashes (–). Use commas, periods, colons, or parentheses instead.
   - Avoid AI-tell phrasing: no "delve", "it's worth noting", "that said", "in today's fast-paced / ever-evolving", "when it comes to", "navigating the landscape", "a testament to", "underscores", "boasts", or reflexive rule-of-three lists. No hedging throat-clearing.
   - Vary sentence length and rhythm. Be concrete and specific (names, numbers, stakes) over vague and smooth. Confident and direct, warm but never sycophantic or peppy.
   - Prefer plain strong verbs and normal punctuation. It should read like a smart friend who happens to be briefing you, not like generated copy.
"""
...
def generate_html_briefing(user_prompt: str, local_now) -> tuple[str, str]:
    ...
    with client.messages.stream(
        ...
        max_tokens=32000,
        # Sonnet 5 defaults adaptive thinking ON. Disable it: Opus generated this
        # briefing with no thinking, so matching that (a) restores the fuller,
        # less-clipped writing that effort=low was truncating, and (b) avoids
        # paying for thinking tokens on what is a direct writing task.
...
def main():
    ...
    news_queries = {
        ...
        # Dedicated WSJ query so the "3 WSJ links" rule gets REAL wsj.com URLs
        # (one macro, one company/industry, one op-ed) instead of substitutes.
        "WSJ":              "wsj.com Wall Street Journal top stories markets business opinion today",
